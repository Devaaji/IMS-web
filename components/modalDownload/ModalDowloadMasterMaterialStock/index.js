...
import useRemoteMasterMaterialStock from '../../hooks/remote/useRemoteMasterMaterialStock';

export function ModalDownloadMasterMaterialStock({
  isOpen,
  onClose,
  filePerXLSX,
  filePerCSV,
  ...
  const NameQuery = 'download-material-master-stock';
  const NameDownloadData = 'MasterMaterialStock';

  const [numberData, setNumberData] = useState();
  const [clickDOwnload, setClickDOwnload] = useState(false);

  const { data: DownloadFullData, isFetched } = useRemoteMasterMaterialStock({
    nameQuery: NameQuery,
