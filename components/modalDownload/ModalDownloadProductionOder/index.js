...
import useRemoteProductionOrder from '../../hooks/remote/useRemoteProductionOrder';

export function ModalDownloadProductionOrder({
  isOpen,
  onClose,
  filePerXLSX,
  filePerCSV,
  ...
  const NameQuery = 'download-full-production-order';
  const NameDownloadData = 'ProductionOrder';

  const [numberData, setNumberData] = useState();
  const [clickDOwnload, setClickDOwnload] = useState(false);

  const { data: DownloadFullData, isFetched } = useRemoteProductionOrder({
    nameQuery: NameQuery,
              ...
              <HStack spacing="3" mt="2">
                <ButtonGroup rounded="xl">
                  <ExportFileCSV
                    ...
                    isLoading={isLoading}
                  >
                    <Button variant="solid-blue" leftIcon={ <FaFileCsv/>  }>CSV</Button>
                  </ExportFileCSV>
                  <Button variant="solid-blue" onClick={filePerXLSX} leftIcon={ <FaFileExcel/> }>XLSX</Button>
                </ButtonGroup>
              </HStack>
            </Box>
            <Box borderTopWidth="2px" py="3">
              ...
              <HStack spacing="3" mt="2">
                <ButtonGroup rounded="xl" colorScheme="green">
                  {isFetched === true ? (
                    ...
                    <Button variant="solid-blue" disabled={true} leftIcon={ <FaFileCsv/> }>CSV</Button>
                  )}
                  {isFetched === true ? (
                    <Button variant="solid-blue" onClick={exportFullFileXLSX} leftIcon={ <FaFileExcel/> }>XLSX</Button>
                  ) : (
                    <Button variant="solid-blue" disabled={true} leftIcon={ <FaFileExcel/> }>XLSX</Button>
                  )}
                </ButtonGroup>
              </HStack>
            </Box>
