...
import useRemotePOItem from '../../hooks/remote/useRemotePOItem';
import useFormFTP from '../../../utils/FormFTP';

const FilterPOItem = ({ data, isLoading }) => {
  const NameDownloadData = 'POItem';
  const DataToDownload = data?.data;
  const { isOpen, onOpen, onClose } = useDisclosure();
  const NameQuery = 'download-full-po-item';

  const [clickDOwnload, setClickDOwnload] = useState(false);

  const { HandleGetFormFTP, isLoadingFTP } = useFormFTP('po-item');

  const { data: DownloadFullData, isFetched } = useRemotePOItem(
    NameQuery,
    99999999999,
    1,
    clickDOwnload,
