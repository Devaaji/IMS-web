...
import useRemoteMasterMaterialBatch from '../../hooks/remote/useRemoteMasterMaterialBatch';

const FilterMaterialBatch = ({ data, isLoading }) => {
  const NameDownloadData = 'MaterialBatch';
  const DataToDownload = data?.data;
  const { isOpen, onOpen, onClose } = useDisclosure();
  const NameQuery = 'download-full-material-batch';

  const [clickDOwnload, setClickDOwnload] = useState(false);

  const { data: DownloadFullData, isFetched } = useRemoteMasterMaterialBatch(
    NameQuery,
    99999999999,
    1,
    clickDOwnload,
...
export default FilterMaterialBatch;
