...
import useRemotePOComponent from '../../hooks/remote/useRemotePOComponent';
import useFormFTP from '../../../utils/FormFTP';

const FilterPOComponent = ({ data, isLoading }) => {
  const NameDownloadData = 'POComponent';
  const DataToDownload = data?.data;
  const { isOpen, onOpen, onClose } = useDisclosure();
  const NameQuery = 'download-full-po-component';

  const [clickDOwnload, setClickDOwnload] = useState(false);

  const { HandleGetFormFTP, isLoadingFTP } = useFormFTP('po-component');

  const { data: DownloadFullData, isFetched } = useRemotePOComponent(
    NameQuery,
    99999999999,
    1,
    clickDOwnload,
...
export default FilterPOComponent;
