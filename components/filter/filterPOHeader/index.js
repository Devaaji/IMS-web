...
import useRemotePOHeader from '../../hooks/remote/useRemotePOHeader';

const FilterPOHeader = ({ data, isLoading }) => {
  const NameDownloadData = 'POHeader';
  const DataToDownload = data?.data;
  const { isOpen, onOpen, onClose } = useDisclosure();
  const NameQuery = 'download-full-po-header';

  const [clickDOwnload, setClickDOwnload] = useState(false);

  const { data: DownloadFullData, isFetched } = useRemotePOHeader(
    NameQuery,
    99999999999,
    1,
    clickDOwnload,
