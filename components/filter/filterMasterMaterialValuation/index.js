...
import useRemoteMasterMaterialValuation from '../../hooks/remote/useRemoteMasterMaterialValuation';

const FilterMasterMaterialValuation = ({ data, isLoading }) => {
  const NameDownloadData = 'MaterialValuation';
  const DataToDownload = data?.data;
  const { isOpen, onOpen, onClose } = useDisclosure();
  const NameQuery = 'download-full-material-valuation';

  const [clickDOwnload, setClickDOwnload] = useState(false);

  const { data: DownloadFullData, isFetched } =
...
export default FilterMasterMaterialValuation;
