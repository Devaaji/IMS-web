...
import useRemoteMasterMaterialVendor from '../../hooks/remote/useRemoteMasterMaterialVendor';

const FilterMasterMaterialVendor = ({ data, isLoading }) => {
  const NameDownloadData = 'MaterialVendor';
  const DataToDownload = data?.data;
  const { isOpen, onOpen, onClose } = useDisclosure();
  const NameQuery = 'download-full-material-vendor';

  const [clickDOwnload, setClickDOwnload] = useState(false);

  const { data: DownloadFullData, isFetched } = useRemoteMasterMaterialVendor(
    NameQuery,
    99999999999,
    1,
    clickDOwnload,
...
export default FilterMasterMaterialVendor;
