...
const useOptionDataMatStockStorage = ({ mutate, filterInput }) => {
  ...
  const { data, ...others } = useInfiniteQuery(
    ...
    async ({ pageParam = 1 }) =>
      await postFetcher(uri, {
        filter: {
          is_transaksi_material_stock_storage: filterInput ? true : false,
          transaksi_material_stock_storage: filterInput,
        },
        limit: 10,
        page: pageParam,
        order: 'transaksi_material_stock_storage',
        sort: 'ASC',
      }),
    {
  ...
  const DataToValues = hasMapDataArr?.map((values) =>
    values
      ?.map((value) => value?.transaksi_material_stock_storage)
...
export default useOptionDataMatStockStorage;
