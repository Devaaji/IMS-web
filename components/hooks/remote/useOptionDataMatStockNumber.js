...
const useOptionDataMatStockNumber = ({ mutate, filterInput }) => {
  ...
  const { data, ...others } = useInfiniteQuery(
    ...
    async ({ pageParam = 1 }) =>
      await postFetcher(uri, {
        filter: {
          is_transaksi_material_stock_material: filterInput ? true : false,
          transaksi_material_stock_material: filterInput,
        },
        limit: 10,
        page: pageParam,
        order: 'transaksi_material_stock_material',
        sort: 'ASC',
      }),
    {
  ...
  const DataToValues = hasMapDataArr?.map((values) =>
    values?.map((value) => value?.transaksi_material_stock_material).filter(removeDupicate)
  );
...
export default useOptionDataMatStockNumber;
